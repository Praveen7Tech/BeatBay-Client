      ...
      <div className="max-w-6xl mx-auto">
        ...
        <form onSubmit={handleSubmit(Onsubmit)} className="flex gap-12">
          ...
          <div className="flex-1">
            <h2 className="text-white text-lg font-semibold mb-6">Track Details</h2>
            ...
            {/* Files */}
            <div className="grid gap-6 mb-6">
              <h2 className="text-white text-lg font-semibold mb-2">Audio Files</h2>
              ...
              <div>
                ...
                >
                  ...
                  <span className="truncate">
                    {trackFileName || "Select audio file (MP3, WAV, FLAC)"}
                  </span>
                </div>
                <Input
                  ref={trackInputRef}
                  type="file"
                  accept="audio/*"
                  onChange={(e) => handleFileChange("trackFile", e.target.files)}
                  className="hidden"
                />
                {errors.trackFile && (
                  <p className="text-red-500 text-xs mt-1">{errors.trackFile.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <span className="truncate">
                    {lrcFileName || "Select .txt / .lrc file"}
                  </span>
                </div>
                <Input
                  ref={lrcInputRef}
                  type="file"
                  accept=".txt"
                  onChange={(e) => handleFileChange("lrcFile", e.target.files)}
                  className="hidden"
                />
                <p className="text-gray-500 text-xs mt-2">
                  LRC format enables time-synced lyrics on streaming platforms
                </p>
                {errors.lrcFile && (
                  <p className="text-red-500 text-xs mt-1">{errors.lrcFile.message}</p>
                )}
              </div>
            </div>
